...
def augment(df_type: DfType, ing_dir: Path = ING_DIR, aug_dir: Path = AUG_DIR):
    with mksb() as sb:
        ...
        # 2. Physics-Informed TDE Augmentation (Train Only)
        if df_type == "train":
            with sb.job("Augmenting TDEs (Physics-Based)"):
                tde_ids = meta_df[meta_df["target"] == 1].index
                # Only augment if we have TDEs
                if len(tde_ids) > 0:
                    z_vals = np.clip(meta_df.loc[tde_ids, "z"].values, 0.001, None)  # pyright: ignore[reportArgumentType, reportCallIssue]
                    d_lum_orig = Planck18.luminosity_distance(z_vals).value  # pyright: ignore[reportAttributeAccessIssue]
                    dist_map = dict(zip(tde_ids, d_lum_orig))

                    obs_groups = obs_df[obs_df["object_id"].isin(tde_ids)].groupby("object_id")

                    new_meta, new_obs = [], []
                    N_AUG = 3  # Generate 3 clones per TDE
                    ...
                    for obj_id in tde_ids:
                        ...
                        # Generate new redshifts
                        new_zs = np.random.uniform(0.01, 0.4, N_AUG)
                        ...
                        for i, z_new in enumerate(new_zs):
                            ...
                            d_new = Planck18.luminosity_distance(z_new).value  # pyright: ignore[reportAttributeAccessIssue]
            ...
            # Gaussian Noise for Robustness (Original Logic)
            with sb.job("Adding Redshift Noise"):
                sigma_z = 0.0  # Default if no test file loaded
                # Try to infer noise from context or set small default
                # (Simplified to avoid circular dependency, assuming ~0.01 or similar)
                sigma_z = 0.01
                noise = np.random.normal(0, sigma_z, len(meta_df))
                meta_df["z"] += noise

        # 3. Derived Columns
        with sb.job("Calculating Derived Columns"):
            z = np.clip(meta_df["z"].values, 1e-5, None)  # pyright: ignore[reportArgumentType, reportCallIssue]
